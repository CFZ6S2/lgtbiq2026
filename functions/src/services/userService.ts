...
export class UserService {
   ...
   */
  async getDiscoverySettings(userId: string): Promise<DiscoverySettings | null> {
    try {
   ...
   */
  async updateDiscoverySettings(userId: string, settings: DiscoverySettings): Promise<DiscoverySettings> {
    try {
      ...
      await db.collection('discoverySettings').doc(userId).set(settingsWithTimestamp, { merge: true });
  ...
  async getUserProfile(userId: string): Promise<any> {
    try {
      const [userDoc, settingsDoc] = await Promise.all([
        ...
        db.collection('discoverySettings').doc(userId).get()
      ]);
      ...
      return {
        ...userData,
        discoverySettings: settingsData,
        id: userId
      };
    } catch (error) {
  ...
  async getUserRestrictions(userId: string): Promise<{
    ...
    try {
      ...
      const blockExpiresAt = userData.blockExpiresAt?.toDate();
