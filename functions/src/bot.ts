...
bot.start((ctx) => {
  ctx.reply(
    '¡Hola! Bienvenido a Prisma, la comunidad LGBTIQ+ segura. 🏳️‍🌈\n\n' +
    ...
    'Usa el comando /register para iniciar.',
    Markup.keyboard([
      ['/register']
    ]).resize()
  );
...
bot.command('register', async (ctx) => {
  ...
  return ctx.reply(
    'Por favor, comparte tu contacto para verificar tu cuenta. ' +
    'Solo usaremos esto para validación de seguridad.',
    Markup.keyboard([
      [Markup.button.contactRequest('📱 Compartir contacto')]
    ]).resize()
  );
});
...
bot.on('contact', async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(
      ...
      'Tu cuenta ha sido creada. Ahora puedes completar tu perfil en nuestra aplicación web.',
      Markup.removeKeyboard()
    );
    
    // Opcional: Enviar botón para abrir la Web App
    return ctx.reply('Completa tu perfil aquí:', Markup.inlineKeyboard([
      Markup.button.url('Abrir Prisma', 'https://lgtbiq26.web.app/register')
    ]));
